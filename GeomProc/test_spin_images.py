...
tm2.vertex = geomproc.apply_transformation(tm1.vertex, np.identity(3), np.zeros((3, 1)))
...
n1 = 100 # We are going to compute a descriptor for each of these points
n2 = 1000 # We are going to use these points only for reference in the descriptor computation
pc1 = tm1.sample(n1)
...
[corr, distances] = geomproc.filter_correspondences(corr=full_matches[:,:2], dist=full_matches[:, 2], keep=0.3)
